 ...
 * Useful to inject mock on unit test and restore safely after done.
 * @function injectmock
...
Injector.prototype = {
  ...
  _indexOfInjection (module, key) {
    const s = this
    for (var i = 0, len = s.injections.length; i < len; i++) {
      let injection = s.injections[ i ]
      let hit = (injection.key === key) && (injection.module === module)
      if (hit) {
        return i
      }
    }
